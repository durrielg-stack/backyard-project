      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === "UPDATE") {
            setTables((prev) =>
              prev.map((t) =>
                t.id === (payload.new as RestaurantTable).id
                  ? (payload.new as RestaurantTable)
                  : t,
              ),
            );
          } else if (payload.eventType === "INSERT") {
            setTables((prev) =>
              sortTables([...prev, payload.new as RestaurantTable]),
            );
          } else if (payload.eventType === "DELETE") {
            setTables((prev) =>
              prev.filter((t) => t.id !== (payload.old as RestaurantTable).id),
            );
          }
        },
      )
