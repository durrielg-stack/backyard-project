...
export function useOrder(tableId: string, staff?: string): UseOrderReturn {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const { data: items, error: iErr } = await sb
        .from("order_items")
        .select(
          "id, menu_item_id, qty, unit_price, unit_cost, modifiers, notes, status, seat, order_type, menu_items(id, name, category, category2)",
        )
          ...
          setLines((prev) =>
            prev.map((l) =>
              l.dbId === (row.id as number)
                ? {
                    ...l,
                    status: row.status as CartLine["status"],
  ...
  const addItem = useCallback(
    async (
      ...
      if (match?.dbId) {
        ...
        setLines((prev) =>
          prev.map((l) =>
            l.lineId === match.lineId ? { ...l, qty: newQty } : l,
          ),
        );
      } else {
        ...
        setLines((prev) =>
          prev.map((l) =>
            l.lineId === tempLineId ? { ...l, dbId: data.id as number } : l,
          ),
        );
      }
    },
  ...
  const updateQty = useCallback(
    async (lineId: string, delta: number) => {
      ...
      setLines((prev) =>
        prev.map((l) => (l.lineId === lineId ? { ...l, qty: newQty } : l)),
      );
    },
    [lines],
  );
  ...
  const closeOrderAndFreeTable = useCallback(
    async (id: number): Promise<boolean> => {
      ...
      const { error: closeErr } = await sb
        ...
        .eq("id", id);
      if (closeErr) {
        setError(`Could not close order: ${closeErr.message}`);
        return false;
      }
      ...
      if (tableErr) {
        setError(
          `Order closed but table status failed to update: ${tableErr.message}`,
        );
      }
      return true;
    },
  ...
  const setNote = useCallback(
    async (lineId: string, note: string) => {
      const sb = getClient();
      const line = lines.find((l) => l.lineId === lineId);
      if (!line) return;
  ...
  const setOrderType = useCallback(
    async (lineId: string, orderType: "dine_in" | "takeout") => {
      const sb = getClient();
      const line = lines.find((l) => l.lineId === lineId);
      if (!line) return;
      ...
      setLines((prev) =>
        prev.map((l) => (l.lineId === lineId ? { ...l, orderType } : l)),
      );
    },
    [lines],
  );
  ...
  const closeOrder = useCallback(
    async (
      ...
      const current = linesRef.current.filter(
        (l) => l.status !== "voided" && l.dbId,
      );
      ...
      if (discountType === "owner_employee") {
        ...
        for (const line of current) {
          await sb
            .from("order_items")
            .update({ unit_price: line.unitCost ?? 0 })
            .eq("id", line.dbId!);
        }
        ...
        for (const u of selected)
          countByLine.set(u.lineId, (countByLine.get(u.lineId) ?? 0) + 1);

        for (const line of current) {
          const discCount = Math.min(
            countByLine.get(line.lineId) ?? 0,
            line.qty,
          );
          ...
          if (discCount === line.qty) {
            ...
            await sb
              .from("order_items")
              .update({ unit_price: discountedPrice })
              .eq("id", line.dbId!);
          } else {
      ...
      if (discountType === "owner_employee")
        noteParts.push("Owner/Employee — billed at cost");
      if (discountType === "senior_pwd")
        noteParts.push(`Senior/PWD ×${seniorCount} — 20% + VAT exempt`);
  ...
  const payPartial = useCallback(
    async (
      ...
      const dbIds = lines
        .filter((l) => lineIds.includes(l.lineId) && l.dbId)
        .map((l) => l.dbId!);
      if (dbIds.length > 0) {
        await sb
          .from("order_items")
  ...
  const moveItems = useCallback(
    async (lineIds: string[], targetTableId: string): Promise<boolean> => {
      const sb = getClient();
      ...
      if (targetOrders && targetOrders.length > 0) {
        ...
        const { data: newOrder, error: noErr } = await sb
          ...
          .single();
        if (noErr || !newOrder) {
          setError(noErr?.message ?? "Could not create order");
          return false;
        }
      ...
      const remainingLines = lines.filter(
        (l) => !lineIds.includes(l.lineId) && l.status !== "voided",
      );
