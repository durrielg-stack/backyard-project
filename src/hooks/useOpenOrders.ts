    ...
    async function load() {
      try {
        ...
        if (openOrders.length > 0) {
          ...
          const { data: items, error: itemsErr } = await (sb as any)
            .from("order_items")
          ...
          for (const item of items ?? []) {
            const mi = Array.isArray(item.menu_items)
              ? item.menu_items[0]
              : item.menu_items;
            const price = (mi?.price as number) ?? 0;
            const prev = orderTotalMap.get(item.order_id as number) ?? 0;
            orderTotalMap.set(
              item.order_id as number,
              prev + price * (item.qty as number),
            );
            ordersWithItems.add(item.order_id as number);
          }

          // An order can only ever get created via the first item add (see
          // useOrder.ensureOrder), so a genuinely active order always has at
          // least one non-voided item. If it has none, its close write most
          // likely failed silently (voidItem/payPartial/moveItems auto-close)
          // and it's a dangling row — don't let it keep a table occupied.
          // Only filter when the items fetch succeeded, so a transient query
          // error can't wrongly free every occupied table.
          const activeOrders = itemsErr
            ? openOrders
            : openOrders.filter((o) => ordersWithItems.has(o.id));
