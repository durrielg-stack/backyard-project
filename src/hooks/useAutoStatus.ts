  ...
  return useMemo(() => {
    ...
    return tables.map((table) => {
      ...
      if (ticketStatus === "late") status = "attention";
        ...
        status = "occupied"; // manual override: no open order but staff marked occupied
      else status = "available";

      const cartStart = cartStartTimes.get(table.id);
      const startMs =
        cartStart ?? (order ? new Date(order.opened_at).getTime() : null);
      const openMin =
        hasActivity && startMs ? Math.floor((now - startMs) / 60_000) : 0;
      ...
      const checkTotal =
        cartTotal > 0 ? cartTotal : (dbTotals.get(table.id) ?? 0);
