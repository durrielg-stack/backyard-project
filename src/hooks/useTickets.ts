...
// aging (amber): drinks 5 min, food 15 min
// late  (red):   drinks 10 min, food 20 min
function getThresholds(category: string): {
  agingSec: number;
  lateSec: number;
} {
  if (BAR_CATS.has(category)) return { agingSec: 300, lateSec: 600 };
  return { agingSec: 900, lateSec: 1200 };
}
  ...
  const fetchAll = useCallback(async () => {
    ...
    for (const row of data) {
      ...
      const mi = Array.isArray(row.menu_items)
        ? row.menu_items[0]
        : row.menu_items;
      const opener = Array.isArray(order?.users)
        ? order.users[0]
        : order?.users;
      ...
      items.push({
        ...
        firedAtMs: row.fired_at
          ? new Date(row.fired_at as string).getTime()
          : null,
