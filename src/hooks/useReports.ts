...
// ── Reducer ───────────────────────────────────────────────────────────────────
type Action =
  { type: "loading" } | { type: "data"; payload: Omit<ReportsData, "loading"> };
  ...
  const fetchAll = useCallback(async () => {
    ...
    if (allOrderIds.length > 0) {
      const { data: lines } = await sb
        .from("order_items")
        .select(
          "id, order_id, qty, unit_price, unit_cost, menu_items(name, category, cost)",
        )
      ...
      for (const row of lines ?? []) {
        ...
        cost +=
          (row.qty as number) *
          ((row as any).unit_cost ?? (mi as any)?.cost ?? 0);
    ...
    const barServed = allServed.filter((i) =>
      BAR_CATS.has(i.menu_items?.category),
    );
    const kitchenServed = allServed.filter(
      (i) => !BAR_CATS.has(i.menu_items?.category),
    );
    ...
    const { data: allExp } = await sb
      .from("daily_expenses")
      .select(
        "id, expense_date, category, description, qty, unit, unit_price, amount, paid_to, created_at",
      )
    ...
    const { data: recentPmts } = await sb
      .from("payments")
      .select(
        `id, order_id, amount, method, processed_at, notes, orders!inner(table_id, opened_by, opened_at, users(name))`,
      )
    ...
    const orderIds: number[] = [
      ...new Set(rp.map((p: any) => p.order_id as number).filter(Boolean)),
    ];
    ...
    function mapTxRow(p: any): TransactionRow {
      ...
      return {
        ...
        isRefund:
          (p.notes as string | null)?.toLowerCase().includes("refund") ?? false,
      };
    }
