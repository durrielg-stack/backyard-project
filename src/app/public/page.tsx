...
/* ---- constants ---- */
const MAPS_URL =
  "https://www.google.com/maps/place/The+Backyard+Project+bar+%2B+kitchen/@15.7143202,120.9070077,20z/data=!4m6!3m5!1s0x3390d716a59781c3:0xb8eb7c82149b36a3!8m2!3d15.7141421!4d120.9063941!16s%2Fg%2F11gmy07jmv";
...
const HERO_TITLES: Record<number, [string, string, string]> = {
  0: [
    "Sunday serenity starts here",
    "Unwind this Sunday",
    "Make Sunday special",
  ],
  1: [
    "Start the week your way",
    "Motivate your Monday",
    "Make Monday your day",
  ],
  2: [
    "We're resting today, back tomorrow",
    "Taking a pause, back tomorrow",
    "Closed today, open for you tomorrow",
  ],
  3: [
    "Midweek magic awaits",
    "Wednesday wonders ahead",
    "Find your midweek spark",
  ],
  4: [
    "Thrive this Thursday",
    "Your Thursday escape",
    "Thursday is the new Friday",
  ],
  5: [
    "Celebrate the weekend early",
    "Feel-good Friday",
    "Your Friyay begins here",
  ],
  6: [
    "Savor your Saturday",
    "Saturday vibes only",
    "Your Saturday adventure awaits",
  ],
};
...
function getHeroTitle(now: Date): string {
  ...
  const slot =
    Math.floor((now.getHours() * 60 + now.getMinutes()) / 30) % titles.length;
  return titles[slot];
}
...
function deriveSummary(
  ...
  const occPct =
    !open || total === 0 ? 0 : Math.round(((total - free) / total) * 100);
...
function IcFb(p: React.SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" {...p}>
      <path
        d="M14.5 8.5H16V5.7h-2.2c-2 0-3.3 1.2-3.3 3.3v1.7H8.3v2.8h2.2V21h2.9v-7.5h2.2l.4-2.8h-2.6V9.3c0-.6.3-.8.9-.8Z"
        fill="currentColor"
      />
    </svg>
  );
  ...
  useEffect(() => {
    ...
    const on = () => {
      ...
      if (!isSolid && y > 360) {
        isSolid = true;
        setSolid(true);
        document.documentElement.classList.add("byp-hero-past");
      } else if (isSolid && y < 200) {
        isSolid = false;
        setSolid(false);
    ...
    <header className={"byp-header" + (solid ? " is-solid" : "")}>
      <div className="byp-header-inner">
        <a className="byp-header-logo" href="#top">
          ...
          <img
            src={theme === "light" ? "/logo-black.svg" : "/byp-logo-topbar.png"}
            alt="The Backyard Project"
          />
          <span className="byp-header-wordmark">
            <span className="byp-header-wordmark-full">
        ...
        <div className="byp-head-right">
          <a className={"byp-head-pill st-" + summary.tone} href="#tables">
            ...
            {summary.tone === "open" ? (
              ...
              <>We&rsquo;re at capacity</>
            ) : (
              <>We&rsquo;re closed</>
            )}
          </a>
          <button
            className="byp-theme-toggle"
            onClick={onToggleTheme}
            aria-label="Toggle theme"
          >
            {theme === "dark" ? (
              <IcSun width={17} height={17} />
            ) : (
              <IcMoon width={17} height={17} />
            )}
          </button>
        </div>
      </div>
        ...
        <div className="byp-hero-copy">
          ...
          <div className="byp-hero-logo-row">
            <img
              ...
              src={theme === "light" ? "/logo-square.png" : "/byp-logo.png"}
              alt="The Backyard Project · bar + kitchen"
            />
            <h1 className="byp-hero-title">
              {heroTitle.slice(0, heroTitle.lastIndexOf(" ") + 1)}
              <span className="byp-amp">
                {heroTitle.slice(heroTitle.lastIndexOf(" ") + 1)}
              </span>
            </h1>
          </div>
          <p className="byp-hero-sub">
            We refresh the page every few seconds, so you know before heading
            over.
          </p>
          <div className="byp-hero-status">
            <span
              className={
                "byp-op-pill " + (summary.open ? "is-open" : "is-closed")
              }
            >
              <span className="byp-dot" style={{ color: "currentColor" }} />
...
function SummaryCard({
  ...
  const cardStyle = preOpen
    ? ({ "--card-accent": preOpen.color } as React.CSSProperties)
    : {};
  ...
  if (summary.open) {
    ...
    statusContent = (
      <div className="byp-sum-number-row">
        <span className="byp-sum-number is-closed">
          Opening
          <br />
          Soon
        </span>
    ...
    <div className={"byp-sum-card st-" + summary.tone} style={cardStyle}>
      <div className="byp-sum-left">
        <div className="byp-sum-eyebrow">
          <span className="byp-dot" style={{ color: "currentColor" }} />
          <span className="byp-eyebrow" style={{ color: "inherit" }}>
            Status
          </span>
        </div>
        {statusContent}
...
function BusyMeter({
  ...
  useEffect(() => {
    ...
    async function fetchAvgOccupancy() {
      ...
      for (let i = 1; i <= 7; i++) {
        const pastMs =
          Date.now() + MANILA_OFFSET_MS - i * 7 * 24 * 60 * 60 * 1000;
      ...
      const oldestStartUTC =
        Date.UTC(oldest.year, oldest.month, oldest.day, 0) - MANILA_OFFSET_MS;
      ...
      const { data } = (await sb
        ...
        data:
          | { table_id: string; opened_at: string; closed_at: string | null }[]
          | null;
      };
      ...
      const dailyPcts = refDays.map(({ year: y, month: m, day: d }) =>
        HOUR_SLOTS.map(({ h }) => {
          ...
          for (const order of data) {
            ...
            const closedMs = order.closed_at
      ...
      const normalized = rawAvg.map((v) =>
        Math.max(15, Math.round(15 + (v / maxRaw) * 81)),
      );
    ...
    fetchAvgOccupancy();
    function onVisible() {
      if (document.visibilityState === "visible") fetchAvgOccupancy();
    }
  ...
  const hourToIdx: Record<number, number> = {
    16: 0,
    17: 1,
    18: 2,
    19: 3,
    ...
    <div className="byp-busy-card">
      ...
      <div className="byp-busy-bars">
        {HOUR_SLOTS.map(({ label }, i) => (
          <div
            key={label}
            className={"byp-busy-col" + (i === activeIdx ? " is-now" : "")}
          >
            <div
              className="byp-busy-bar"
              style={{ height: Math.round((barData[i] / 100) * 64) + "px" }}
            />
...
function TablesSection({
  ...
  tables: { id: string; label: string; status: Status }[];
}) {
  return (
    <section className="byp-block byp-shell" id="tables">
      ...
      <div className="byp-tables-grid">
        {tables.map((t) => (
          <TableTile key={t.id} label={t.label} status={t.status} />
        ))}
      </div>
    </section>
  );
  ...
  return (
    <section className="byp-block byp-shell" id="menu">
      ...
      <div className="byp-menu-rail">
        <figure
          className="byp-menu-page"
          onClick={() => onZoom("/menu-food.jpg")}
          role="button"
          tabIndex={0}
          onKeyDown={(e) => e.key === "Enter" && onZoom("/menu-food.jpg")}
        ...
        >
          ...
          <figcaption>
            <span>Food</span>
            <IcZoom width={16} height={16} />
          </figcaption>
        </figure>
        <figure
          className="byp-menu-page"
          onClick={() => onZoom("/menu-drinks.jpg")}
          role="button"
          tabIndex={0}
          onKeyDown={(e) => e.key === "Enter" && onZoom("/menu-drinks.jpg")}
    ...
    <section className="byp-block byp-shell" id="hours">
      <div className="byp-hours-card">
        <div className="byp-hours-left">
          ...
          <h2 className="byp-block-title">
            Open daily
            <br />
            from 4&nbsp;PM
          </h2>
          <p className="byp-hours-note">
            We pour until midnight, seven nights a week — except Tuesdays, when
            the backyard rests.
          </p>
          <span
            className={"byp-op-pill " + (openNow ? "is-open" : "is-closed")}
          >
            <span className="byp-dot" style={{ color: "currentColor" }} />
        ...
        <ul className="byp-hours-list">
          {HOURS.map(([day, hrs], i) => (
            <li
              ...
              className={
                (i === todayIdx ? "is-today " : "") +
                (hrs === "Closed" ? "is-closed" : "")
              }
            >
              <span className="byp-hd-day">
                {day}
                {i === todayIdx && <em>Today</em>}
              </span>
  ...
  return (
    <section className="byp-block byp-shell" id="gallery">
      ...
      <div className="byp-gallery-grid">
        ...
        {[
          {
            src: "/gallery-string-lights.jpg",
            alt: "String lights and drinks at The Backyard Project",
          },
          {
            src: "/gallery-cocktails.jpg",
            alt: "Signature cocktails at The Backyard Project",
          },
      ...
      <div className="byp-visit-grid">
        <div className="byp-visit-info">
          ...
          <ul className="byp-visit-list">
            <li>
              <span className="byp-vi-ic">
                <IcPin width={20} height={20} />
              </span>
              <span>
                <b>The Backyard Project · bar + kitchen</b>
                <br />
                <span className="byp-vi-dim">
                  Mabini St Ext, Muñoz, 3119 Nueva Ecija
                </span>
              </span>
            </li>
            <li>
          ...
          <div className="byp-visit-actions">
            ...
            <div className="byp-socials">
              <a
                href="https://www.instagram.com/thebackyardprojectph"
                target="_blank"
                rel="noreferrer"
              ...
              <a
                href="https://www.facebook.com/thebackyardprojectph"
                target="_blank"
                rel="noreferrer"
                aria-label="Facebook"
                className="byp-soc"
        ...
        <div className="byp-visit-map">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="byp-map-img-link"
          >
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src="/stylized-map.png"
              alt="Map to The Backyard Project"
              className="byp-map-img"
            />
          </a>
          <a
            className="byp-map-cta"
            href={MAPS_URL}
            target="_blank"
            rel="noreferrer"
          >
...
function SiteFooter() {
  return (
    <footer className="byp-footer">
      ...
      <p className="byp-fine">
        © {new Date().getFullYear()} The Backyard Project · bar + kitchen
      </p>
    </footer>
  );
}
...
function Lightbox({
  ...
  return (
    <div className="byp-lightbox" onClick={onClose}>
      <button className="byp-lb-close" aria-label="Close" onClick={onClose}>
        ✕
      </button>
      {/* eslint-disable-next-line @next/next/no-img-element */}
...
export default function TablesPage() {
  ...
  useEffect(() => {
    ...
    sb.from("restaurant_tables")
      ...
      .then(({ data }) => {
        setRawTables(sortById(data ?? []));
        setUpdatedAt(Date.now());
      });
    const ch = sb
      .channel("public-tables-v2")
      .on(
        "postgres_changes",
