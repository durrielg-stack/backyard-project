...
export default function OrderView({
  ...
  function handleSplit() {
    if (lines.length > 0) setModal({ kind: "split" });
  }
  function handleCharge() {
    if (lines.length > 0)
      setModal({ kind: "pay", payAmount: total, splitLineIds: null });
  }
  function handleBillItem(lineId: string) {
    ...
    setModal({
      kind: "pay",
      payAmount: line.unitPrice * line.qty,
      splitLineIds: [lineId],
      singleItem: true,
    });
  }
  ...
  function handleSplitConfirm(result: SplitResult) {
    if (result.mode === "equally") {
      ...
      setModal({
        kind: "pay",
        payAmount: amount,
        splitLineIds: lines.map((l) => l.lineId),
      });
      ...
      const firstSeat = Math.min(
        ...Object.values(result.seatMap).filter((s) => s > 0),
      );
      const seatLines = lines.filter(
        (l) => result.seatMap![l.lineId] === firstSeat,
      );
      const amount = seatLines.reduce((s, l) => s + l.unitPrice * l.qty, 0);
      setModal({
        kind: "pay",
        payAmount: amount,
        splitLineIds: seatLines.map((l) => l.lineId),
      });
    }
  }
      ...
      >
        ...
        >
          <OrderPanel
            ...
            onMove={
              lines.length > 0 ? () => setModal({ kind: "move" }) : undefined
            }
