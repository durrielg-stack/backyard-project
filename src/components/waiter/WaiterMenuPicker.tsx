...
export default function WaiterMenuPicker({
  tableId,
  waiterId,
  waiterName,
  onBack,
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((item) => {
      const matchCat =
        ...
        g.cats.some(
          (c) =>
            item.category === c || item.category2 === c || item.category3 === c,
        );
      const matchQ =
    ...
    setPending((prev) => {
      ...
      if (newQty <= 0) {
        ...
        next.set(item.id, {
          item,
          qty: newQty,
          orderType: cur?.orderType ?? "dine_in",
        });
      }
      return next;
    });
  ...
  function toggleOrderType(itemId: string) {
    setPending((prev) => {
      ...
      next.set(itemId, {
        ...cur,
        orderType: cur.orderType === "dine_in" ? "takeout" : "dine_in",
      });
      return next;
    });
  }
  ...
  return (
    ...
    >
      ...
      >
        <button
          ...
          style={{
            ...
            color: sending ? T.textMute : T.info,
            fontSize: 14,
            fontFamily: "inherit",
            padding: "10px 8px",
            minHeight: 44,
      ...
      {/* Category tabs */}
      <div
        className="bp-scroll-x bp-no-scrollbar"
        style={{
          display: "flex",
          gap: 6,
          padding: "10px 16px",
          ...
          >
            ...
            {pendingList.map(({ item, qty, orderType }) => (
              ...
              >
                ...
                >
                  <span style={{ color: T.text, flex: 1, minWidth: 0 }}>
                    {item.name}
                  </span>
                  <button
                    ...
                    style={{
                      ...
                      background:
                        orderType === "takeout" ? T.info + "20" : "transparent",
