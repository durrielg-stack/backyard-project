...
> = {
  Beer: {
    containerUnits: ["case", "box"],
    defaultSize: "24",
    baseUnit: "bottle",
    itemLabel: "Beer Item",
    uom: BEER_UOM_OPTIONS,
  },
  Cigarettes: {
    containerUnits: ["pack"],
    defaultSize: "20",
    baseUnit: "stick",
    itemLabel: "Cigarette Item",
    uom: CIG_UOM_OPTIONS,
  },
};
...
function catColor(T: ReturnType<typeof useTheme>["T"]): Record<string, string> {
  return {
    ...
    Food: T.ok,
    Beer: T.warn,
    "Cocktails/Hard": T.accent,
    "Non-Alcohol": T.info,
    Cigarettes: T.textMute,
  };
}
          ...
          <div>
            ...
            <span
              style={{
                marginLeft: 8,
                fontSize: 10,
                color: catColor(T)[s.category] ?? T.textMute,
              }}
            >
              {s.category}
            </span>
...
export default function ExpensesView({ role = "manager" }: { role?: string }) {
  ...
  const {
    ...
    toggle: sortToggle,
    icon: sortIcon,
  } = useSortable(rows, "expenseDate" as keyof ExpenseRow, "desc");
  ...
  // Load presets once
  useEffect(() => {
    sb.from("expense_presets")
      .select("name,category,default_cost,menu_item_id")
      .order("name")
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  useEffect(() => {
    (async () => {
      const [{ data: items }, { data: comps }] = await Promise.all([
        sb
          .from("menu_items")
          .select("id,name,category2")
          .in("category2", Object.keys(RESTOCK_CFG))
          .order("name"),
        sb.from("inventory_compositions").select("sold_menu_item_id"),
      ]);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const composedIds = new Set(
        (comps ?? []).map((c: any) => c.sold_menu_item_id),
      );
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      setLinkItems(
        (items ?? [])
          .filter((r: any) => !composedIds.has(r.id))
          .map((r: any) => ({
            id: r.id,
            name: r.name,
  ...
  function handleDescKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    ...
    if (e.key === "Enter" && suggestionIdx >= 0) {
      e.preventDefault();
      applyPreset(suggestions[suggestionIdx]);
    }
    if (e.key === "Escape") {
      setShowSuggestions(false);
    }
  ...
  async function addExpense() {
    ...
    const invQty =
      fMenuItemId && cfg ? (isContainer ? qty * caseSize : qty) : null;
    ...
    >
      ...
      {showForm &&
        (() => {
          ...
          const canSave =
            fDesc.trim() && (up != null ? up > 0 : parseFloat(fAmt) > 0);
          return (
            <div
              className="bp-no-scrollbar"
              style={{
            ...
            >
              ...
              >
                ...
                {isOwner && (
                  <div>
                    ...
                    <input
                      ...
                      max={today}
                      onChange={(e) =>
                        e.target.value && setFDate(e.target.value)
                      }
                      style={{
                        width: "100%",
                        fontFamily: T.mono,
                        fontSize: 12,
                        background: fDate !== today ? `${T.warn}18` : T.surface,
                        border: `1px solid ${fDate !== today ? T.warn : T.line2}`,
                        color: T.text,
                        borderRadius: T.radius,
                        padding: "6px 8px",
                        outline: "none",
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const c = RESTOCK_CFG[e.target.value];
                      if (c) setFCaseSize(c.defaultSize);
                    }}
                    style={{
                      width: "100%",
                      fontFamily: "inherit",
                  ...
                  <input
                    ...
                    onFocus={() =>
                      ...
                      setSuggestions(
                        presets
                          .filter((p) =>
                            p.name.toLowerCase().includes(fDesc.toLowerCase()),
                          )
                          .slice(0, 8),
                      )
                    }
                ...
                <div>
                  ...
                  >
                    <option value="">—</option>
                    {(RESTOCK_CFG[fCat]?.uom ?? UOM_OPTIONS).map((u) => (
                ...
                <div>
                  ...
                  >
                    Total Price{" "}
                    {up != null ? (
                      <span style={{ color: T.accent }}>auto</span>
                    ) : (
                      "*"
                    )}
                  </div>
                  <input
                    ...
                    onChange={(e) => {
                      if (up == null) setFAmt(e.target.value);
                    }}
                    readOnly={up != null}
                    placeholder="0.00"
                    type="number"
              ...
              {(() => {
                const cfg = RESTOCK_CFG[fCat];
                if (!cfg) return null;
                const catItems = linkItems.filter(
                  (it) => it.category2 === fCat,
                );
                  ...
                  >
                    ...
                    >
                      <div>
                        ...
                        <select
                          value={fMenuItemId}
                          onChange={(e) => setFMenuItemId(e.target.value)}
                          style={{
                            width: "100%",
                            fontFamily: "inherit",
                            fontSize: 12,
                      ...
                      {isContainer && (
                        <div>
                          ...
                          <input
                            value={fCaseSize}
                            onChange={(e) => setFCaseSize(e.target.value)}
                            disabled={!fMenuItemId}
                            type="number"
                            min="1"
                            step="1"
                            style={{
                              ...
                              fontFamily: T.mono,
                              fontSize: 13,
                              background: fMenuItemId ? T.surface : T.chip,
                              border: `1px solid ${T.line2}`,
                              color: T.text,
                              borderRadius: T.radius,
                              padding: "6px 8px",
                    ...
                    {fMenuItemId && unitsToAdd > 0 && (
                      ...
                      >
                        → Adds {unitsToAdd} {cfg.baseUnit}
                        {unitsToAdd === 1 ? "" : "s"} to {selectedItem?.name} (
                        {isContainer
                          ? `${qty} ${fUnit} × ${caseSize}`
                          : `${qty} ${cfg.baseUnit}`}
                        {qty === 1 && !isContainer ? "" : "s"})
      ...
      >
        ...
        >
          <div
            style={{
              display: "grid",
              gridTemplateColumns: "90px 80px 120px 1fr 180px 100px 36px",
              padding: "0 24px",
              height: 36,
              alignItems: "center",
              borderBottom: `1px solid ${T.line}`,
          ...
          >
            {(
              [
                ...
                ["Amount", "amount"],
                ["", null],
              ] as [string, keyof ExpenseRow | null][]
            ).map(([h, k]) =>
              k ? (
                <button
                  key={h}
          ...
          {loading ? (
            ...
            filteredRows.map((row, i) => {
              ...
              const qtyPart =
                row.qty % 1 === 0 ? String(row.qty) : row.qty.toFixed(3);
              ...
              return (
                ...
                >
                  ...
                  >
                    {row.expenseDate.slice(5)}
                  </span>
                  <span
                    style={{
                      fontSize: 11,
                      fontWeight: 600,
                      color: catColor(T)[row.category] ?? T.textDim,
                    }}
                  >
                    {row.category}
                  </span>
      ...
      {rows.length > 0 && (
        ...
        >
          {search && (
            <span
              style={{ fontSize: 11, color: T.textMute, fontFamily: T.mono }}
            >
