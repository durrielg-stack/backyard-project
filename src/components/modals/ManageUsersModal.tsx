...
export default function ManageUsersModal({
  ...
  async function handleToggleStatus(user: StaffUser) {
    ...
    const { error } = await setUserStatus(
      await authToken(),
      user.id,
      newStatus,
    );
  ...
  async function handleResetPassword() {
    ...
    setResetError(null);
    const { error } = await resetOtherUserPassword(
      await authToken(),
      resetTarget.id,
      resetPw,
    );
  ...
  function inputStyle(err?: boolean): React.CSSProperties {
    return {
      ...
      background: T.bg,
      border: `1px solid ${err ? T.bad : T.line2}`,
      color: T.text,
      fontFamily: "inherit",
        ...
        {displayed.map((user) => (
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div
                style={{
                  ...
                  fontWeight: 600,
                  color:
                    user.account_status === "disabled" ? T.textMute : T.text,
                }}
              >
                {user.name}
              </div>
            ...
            <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
              {user.id !== actorId &&
                !(actorRole === "owner" && user.role === "owner") && (
                  <button
                    onClick={() => {
                      setResetTarget(user);
                      setResetPw("");
              ...
              {isOwner && user.id !== actorId && (
                <button
                  ...
                  style={{
                    ...
                    fontWeight: 600,
                    background:
                      user.account_status === "active" ? T.surface2 : T.surface,
          ...
          {!showAddForm ? (
            ...
            >
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button
                  type="submit"
                  disabled={addLoading || !addName || !addPw}
                  style={{
                    flex: 1,
                    padding: "9px",
                    fontSize: 13,
                    fontWeight: 700,
                    background:
                      addLoading || !addName || !addPw ? T.chip : T.accent,
                    color:
