...
function ByItemTab({
  ...
  return (
    <div
      style={{ display: "flex", gap: 0, height: "clamp(240px, 45vh, 380px)" }}
    >
      {/* Items list */}
      <div
        style={{
...
function BySeatTab({
  ...
  seats: number;
  total: number;
  onConfirm: (map: Record<string, number>) => void;
}) {
  const { T } = useTheme();
  ...
  const unassigned = lines.filter(
    (l) => !seatMap[l.lineId] || seatMap[l.lineId] === 0,
  );
