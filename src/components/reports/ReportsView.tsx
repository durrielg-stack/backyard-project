...
function SalesKpiStrip({
  ...
  const kpis = [
    ...
    },
    {
      label: `Net · ${suffix}`,
      value: fp(net),
      note: revenue > 0 ? `${((net / revenue) * 100).toFixed(1)}% margin` : "—",
    ...
    {
      label: `Cashflow · ${suffix}`,
      value: fp(cashflow),
      note: net > 0 ? `${((cashflow / net) * 100).toFixed(1)}% of net` : "—",
      color: cashflow >= 0 ? T.ok : T.bad,
    },
    {
      label: "Avg Order",
      value: avgOrder > 0 ? fp(avgOrder) : "—",
      note: `${txCount} closed`,
      color: T.textDim,
    },
    {
...
function ExpensesKpiStrip({
  ...
  return (
    <div
      className="bp-no-scrollbar"
      style={{
        display: isMobile2 ? "flex" : "grid",
        gridTemplateColumns: isMobile2 ? undefined : "repeat(6, 1fr)",
        overflowX: isMobile2 ? "auto" : undefined,
        touchAction: "pan-x pan-y",
        overscrollBehaviorX: "contain",
        overscrollBehaviorY: "none",
        WebkitOverflowScrolling: "touch",
      ...
      <div style={{ position: "relative", height: chartHeight }}>
        {[0.25, 0.5, 0.75, 1].map((pct) => (
          ...
          >
            ...
            >
              ₱
              {(maxVal * pct).toLocaleString("en-PH", {
                ...
                {bar.value > 0 && (
                  <div
                    style={{
                      ...
                      fontSize: 9,
                      fontWeight: 600,
                      color: bar.isPeak ? color : T.textMute,
                      fontVariantNumeric: "tabular-nums",
                      marginBottom: 2,
                      whiteSpace: "nowrap",
                    }}
  ...
  if (isMobile) {
    return (
      ...
      >
        <div
          style={{
            minWidth: minChartWidth || "100%",
            width: minChartWidth > 0 ? `max(100%, ${minChartWidth}px)` : "100%",
          }}
        >
          {chartContent}
        </div>
...
function ExpensesChartPanel({
  ...
  return (
    ...
    >
      <PanelHd
        title="Expenses"
        badge={`₱${expenses.toLocaleString("en-PH", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
        badgeColor={T.bad}
      />
      <BarChart
        bars={expenseDayBars}
        barColor={T.bad}
        chartHeight={mobile ? 160 : 140}
      />
    </div>
  );
}

// ── Sales transactions / per-item list ────────────────────────────────────────
const TX_COLS = "60px 64px 46px 1fr 28px 90px 64px";
const TX_HDRS = ["Time", "ID", "Tbl", "Server", "×", "Total", "Pay"];
const ITEM_COLS = "60px 46px 120px 1fr 32px 90px";
const ITEM_HDRS = ["Time", "Tbl", "Category", "Item", "Qty", "Amount"];
              ...
              >
                ...
                <span
                  style={{
                    fontSize: 10,
                    fontWeight: 600,
                    color: getCatColor(T)[row.category] ?? T.textDim,
                  }}
                >
                  {row.category}
                </span>
