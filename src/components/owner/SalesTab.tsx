...
export default function SalesTab() {
  ...
  const [tableView, setTableView] = useState<"orders" | "summary">("orders");
    ...
    async (start: string, end: string) => {
      ...
      for (let from = 0; ; from += PAGE) {
        const { data, error } = await sb
          .from("order_items")
          .select(
            "id, order_id, qty, unit_price, unit_cost, status, fired_at, completed_at, menu_items(name, category, cost)",
          )
      ...
      setLines(
        rows.map((r: SaleRow) => {
          ...
          const serveMin =
            r.fired_at && r.completed_at
              ? Math.round(
                  (new Date(r.completed_at).getTime() -
                    new Date(r.fired_at).getTime()) /
                    60000,
                )
              : null;
          ...
          return {
            id: r.id,
            time: orderTimeMap[r.order_id] ?? "—",
            date: orderDateMap[r.order_id] ?? "—",
            ts: orderTsMap[r.order_id] ?? 0,
  ...
  const rawItemSummaries: ItemSummary[] = Array.from(
    itemSummaryMap.values(),
  ).map((s) => {
    // average serve time only over lines that have it
    const serveLines = filteredLines.filter(
      (l) => l.itemName === s.itemName && l.serveMin != null,
    );
    const avgServeMin =
      serveLines.length > 0
        ? Math.round(
            serveLines.reduce((acc, l) => acc + l.serveMin!, 0) /
              serveLines.length,
          )
        : null;
    return {
      ...s,
      margin: s.gross > 0 ? (s.net / s.gross) * 100 : 0,
      avgServeMin,
    };
  });
  ...
  const {
    ...
    icon: summaryIcon,
  } = useSortable<ItemSummary>(rawItemSummaries, "gross", "desc");
    ...
    >
      ...
      {isMobile ? (
          ...
          <div style={{ padding: "0 16px 8px", position: "relative" }}>
            <input
              ...
              style={{
                ...
                border: `1px solid ${search ? T.accent : T.line2}`,
                color: T.text,
                borderRadius: T.radius,
        ...
        <SectionHd
          ...
          badge={
            lines.length > 0
              ? `${filteredLines.length}${search ? `/${lines.length}` : ""} items`
              : undefined
          }
          action={
            ...
            >
              ...
              <tbody>
                ...
                >
                  ...
                  <td style={td("right", { fontWeight: 700, color: T.text })}>
                    {fmtPeso(totalGross)}
                  </td>
                  <td
                ...
                <thead>
                  <tr>
                    <th
                      ...
                      title="When the order was closed/billed, not when it was opened"
                    >
                      Time
                    </th>
                    {nav.mode !== "today" && (
                      <th style={th("left", { minWidth: 64 })}>Date</th>
                    )}
                    ...
                    .map((l, i) => {
                      ...
                      return (
                        <tr key={l.id} style={{ background: rowBg }}>
                          ...
                          <td style={td("left", { color: T.textDim })}>
                            {l.tableId}
                          </td>
                          <td style={td("left", { color: T.textDim })}>
                            {l.category}
              ...
              <table style={{ borderCollapse: "collapse", minWidth: 900 }}>
                <thead>
                  <tr>
                    ...
                    >
                      <button
                        style={sortBtn("category")}
                        onClick={() => summaryToggle("category")}
                      >
                        Category
                        <span style={{ fontSize: 8, opacity: 0.7 }}>
                          {summaryIcon("category")}
                        </span>
                      </button>
                    </th>
                    <th style={th("left", { minWidth: 220 })}>
                      <button
                        style={sortBtn("itemName")}
                        onClick={() => summaryToggle("itemName")}
                      >
                        Item Name
                        <span style={{ fontSize: 8, opacity: 0.7 }}>
                          {summaryIcon("itemName")}
                        </span>
                      </button>
                    </th>
                    <th style={th("right", { minWidth: 64 })}>
                      <button
                        style={sortBtn("totalQty", "right")}
                        onClick={() => summaryToggle("totalQty")}
                      >
                        Qty
                        <span style={{ fontSize: 8, opacity: 0.7 }}>
                          {summaryIcon("totalQty")}
                        </span>
                      </button>
                    </th>
                    <th style={th("right", { minWidth: 120 })}>
                      <button
                        style={sortBtn("gross", "right")}
                        onClick={() => summaryToggle("gross")}
                      >
                        Gross
                        <span style={{ fontSize: 8, opacity: 0.7 }}>
                          {summaryIcon("gross")}
                    ...
                    <th style={th("right", { minWidth: 120 })}>
                      <button
                        style={sortBtn("cost", "right")}
                        onClick={() => summaryToggle("cost")}
                      >
                        Cost
                        <span style={{ fontSize: 8, opacity: 0.7 }}>
                          {summaryIcon("cost")}
                    ...
                    <th style={th("right", { minWidth: 120 })}>
                      <button
                        style={sortBtn("net", "right")}
                        onClick={() => summaryToggle("net")}
                      >
                        Net
                        <span style={{ fontSize: 8, opacity: 0.7 }}>
                          {summaryIcon("net")}
                    ...
                    <th style={th("right", { minWidth: 110 })}>
                      <button
                        style={sortBtn("avgServeMin", "right")}
                        onClick={() => summaryToggle("avgServeMin")}
                      >
                        Avg Fire→Serve
                        <span style={{ fontSize: 8, opacity: 0.7 }}>
                          {summaryIcon("avgServeMin")}
                        </span>
                      </button>
                    </th>
                    <th style={th("right", { minWidth: 90 })}>
                      <button
                        style={sortBtn("margin", "right")}
                        onClick={() => summaryToggle("margin")}
                      >
                        Margin %
                        <span style={{ fontSize: 8, opacity: 0.7 }}>
                          {summaryIcon("margin")}
                  ...
                  {sortedSummary.map((s, i) => {
                    ...
                    return (
                      <tr key={s.itemName} style={{ background: rowBg }}>
                        ...
                        <td
                          style={td("right", {
                            color: s.avgServeMin != null ? T.info : T.textMute,
                          })}
                        >
