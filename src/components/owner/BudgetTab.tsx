...
function buildLedger(
  ...
  const allDates = Array.from(
    new Set([...Object.keys(allIncoming), ...Object.keys(allExpenses)]),
  ).sort();
  ...
  for (const date of dates) {
    ...
    const hasActivity =
      Object.values(allIncoming[date] ?? {}).some((v) => v > 0) ||
    ...
    for (const c of BUDGET_CATS) {
      ending[c.id] =
        starting[c.id] + (incoming[c.id] ?? 0) - (expenses[c.id] ?? 0);
...
export default function BudgetTab() {
  ...
  const [seedInputs, setSeedInputs] = useState<Record<string, string>>(() =>
    Object.fromEntries(BUDGET_CATS.map((c) => [c.id, "0"])),
  );
  ...
  const fetchSeedAndOpex = useCallback(async () => {
    const [{ data: seedRows }, { data: opexItemRows }, { data: opexCfgRows }] =
      await Promise.all([
        sb
          .from("budget_seed")
          .select("*")
  ...
  const fetchLedger = useCallback(async () => {
    ...
    const seedStartISO = seed?.date
      ? new Date(seed.date).toISOString()
      : "1970-01-01";
    ...
    for (let from = 0; ; from += PAGE) {
      ...
      if (error) {
        console.error("[BudgetTab/ledger] orders error", error);
        break;
      }
    ...
    for (const o of allOrderRows)
      orderDateMap[o.id] = shiftLocalDate(new Date(o.opened_at));
    ...
    if (orderIds.length > 0) {
      for (let from = 0; ; from += PAGE) {
        const { data, error } = await sb
          ...
          .range(from, from + PAGE - 1);
        if (error) {
          console.error("[BudgetTab/ledger] items error", error);
          break;
        }
        if (!data || data.length === 0) break;
    ...
    for (let from = 0; ; from += PAGE) {
      const { data, error } = await sb
        ...
        .range(from, from + PAGE - 1);
      if (error) {
        console.error("[BudgetTab/ledger] expenses error", error);
        break;
      }
      if (!data || data.length === 0) break;
    ...
    for (const r of expRows ?? []) {
      ...
      if (!cat) continue;
      if (!allExpenses[r.expense_date])
        allExpenses[r.expense_date] = emptyBycat();
      allExpenses[r.expense_date][cat] =
        (allExpenses[r.expense_date][cat] ?? 0) + r.amount;
    }

    setLedgerRows(
      buildLedger(allIncoming, allExpenses, seed, opexItems, opexConfigs),
    );
  ...
  const fetchDay = useCallback(
    async (d: string) => {
      ...
      for (const priorDate of priorDates) {
        ...
        priorIncoming.opex =
          (priorIncoming.opex ?? 0) +
          Math.ceil(computeDailyOpex(opexItems, priorCfg));
      }
      ...
      for (let from = 0; ; from += PAGE) {
        const { data, error } = await sb
          ...
          .select("category, amount")
          .lt("expense_date", d)
          .gte("expense_date", seedStart)
          .range(from, from + PAGE - 1);
        if (error) {
          console.error("[BudgetTab] prior expenses error", error);
          break;
        }
        if (!data || data.length === 0) break;
  ...
  const getStarting = (catId: string) =>
    (prevData.incoming[catId] ?? 0) - (prevData.expenses[catId] ?? 0);
  const totalStarting = BUDGET_CATS.reduce((s, c) => s + getStarting(c.id), 0);
  const totalIncoming = BUDGET_CATS.reduce(
    (s, c) => s + (dayData.incoming[c.id] ?? 0),
    0,
  );
  const totalExpenses = BUDGET_CATS.reduce(
    (s, c) => s + (dayData.expenses[c.id] ?? 0),
    0,
  );
    ...
    >
      {isMobile ? (
        <div style={{ flexShrink: 0, borderBottom: `1px solid ${T.line}` }}>
          ...
          >
            ...
            >
              {budgetView === "day"
                ? `Ending ${fmtSign(totalEnding)}`
                : `${ledgerRows.length} days`}
            </span>
          </div>
      ...
      {!seed && !showSeedForm && (
        <div
          style={{
            ...
            borderBottom: `1px solid ${T.warn}44`,
            display: "flex",
            alignItems: "center",
            gap: 12,
            flexShrink: 0,
      ...
      {budgetView === "day" && (
        <>
                    ...
                    >
                      ...
                      <span
                        style={{
                          fontFamily: T.mono,
                          fontSize: 13,
                          color: incoming > 0 ? T.ok : T.textMute,
                          fontVariantNumeric: "tabular-nums",
                        }}
                      >
                        {fmtSign(incoming)}
                      ...
                      <span
                        style={{
                          fontFamily: T.mono,
                          fontSize: 13,
                          color: expenses > 0 ? T.bad : T.textMute,
                          fontVariantNumeric: "tabular-nums",
                        }}
                      >
                        {fmtSign(expenses)}
                ...
                >
                  ...
                  <span
                    style={{
                      ...
                      fontSize: 13,
                      fontWeight: 700,
                      color: totalStarting >= 0 ? T.ok : T.bad,
                      fontVariantNumeric: "tabular-nums",
                    }}
                  >
                    {fmtSign(totalStarting)}
                  </span>
                  <span
                    style={{
                      fontFamily: T.mono,
                  ...
                  <span
                    style={{
                      ...
                      fontSize: 16,
                      fontWeight: 700,
                      color: totalEnding >= 0 ? T.ok : T.bad,
                      fontVariantNumeric: "tabular-nums",
                    }}
                  >
                    {fmtSign(totalEnding)}
          ...
          >
            COGS pulled from item cost × qty sold · OPEX allocation from
            calculator · Expenses from daily logs · Starting balance rolls from
            seed
          </div>
        </>
      )}
      ...
      {budgetView === "ledger" && (
        ...
        >
          {ledgerRows.length === 0 ? (
            ...
            >
              No data yet — bill out some orders to see data here.
            </div>
          ) : (
            (() => {
                  ...
                  {reversed.map((row, ri) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {GROUPS_L.map((g) => {
                          const groupData = row[g.key] as Record<
                            string,
                            number
                          >;
                          const total =
