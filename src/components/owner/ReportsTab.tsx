...
export default function ReportsTab() {
  ...
  const fetchAll = useCallback(
    async (startISO: string, endISO: string, mode: ViewMode) => {
      setLoading(true);
      ...
      const hourGross: Record<number, number> = {};
      const hourCost: Record<number, number> = {};
      const dayGross: Record<string, number> = {};
      const dayCost: Record<string, number> = {};
      let gTotal = 0;
      let cTotal = 0;
      const txN = orderIds.length;
      const itemAgg: Record<
        string,
        { qty: number; rev: number; cost: number }
      > = {};
      ...
      if (orderIds.length > 0) {
        const { data: allLines } = await sb
          .from("order_items")
          .select(
            "order_id, qty, unit_price, unit_cost, menu_items(name, category, cost)",
          )
        ...
        for (const row of allLines ?? []) {
          ...
          gTotal += val;
          cTotal += rc;
          dayGross[dk] = (dayGross[dk] ?? 0) + val;
          dayCost[dk] = (dayCost[dk] ?? 0) + rc;

          if (mode === "today") {
            const h = openedAt.getHours();
            hourGross[h] = (hourGross[h] ?? 0) + val;
            hourCost[h] = (hourCost[h] ?? 0) + rc;
          }

          if (!itemAgg[name]) itemAgg[name] = { qty: 0, rev: 0, cost: 0 };
          itemAgg[name].qty += row.qty;
          itemAgg[name].rev += val;
          itemAgg[name].cost += rc;
          if (!catAgg[cat]) catAgg[cat] = { gross: 0, cost: 0 };
          catAgg[cat].gross += val;
          catAgg[cat].cost += rc;
        }
      }
      ...
      for (const r of expData ?? []) {
        expDayBuckets[r.expense_date] =
          (expDayBuckets[r.expense_date] ?? 0) + r.amount;
        expCatBuckets[r.category] = (expCatBuckets[r.category] ?? 0) + r.amount;
        expTotal += r.amount;
      }
      setExpenses(expTotal);
      setExpCat(
        Object.entries(expCatBuckets)
      ...
      for (const p of pmts ?? []) {
        mm[p.method] = (mm[p.method] ?? 0) + p.amount;
      }
      setMethodMap(mm);

      const { data: voidedData } =
        orderIds.length > 0
          ? await sb
              .from("order_items")
              .select(
                "id, qty, unit_price, void_reason, order_id, menu_items(name), orders!inner(table_id, opened_at)",
              )
              .eq("status", "voided")
              .in("order_id", orderIds)
      ...
      setVoidedAmount(
        vi.reduce((s: number, r: any) => s + r.qty * r.unit_price, 0),
      );
      setVoidedItems(
        vi.map((row: any) => {
  ...
  const suffix =
    nav.mode === "today" ? "Today" : nav.mode === "week" ? "Week" : "Month";
    ...
    >
      ...
      {(() => {
        const kpis = [
          ...
          },
          {
            ...
            sub:
              gross > 0
                ? `${((cost / gross) * 100).toFixed(1)}% of gross`
                : "—",
            color: T.textDim,
          },
          {
            ...
            value: fmtPeso(net),
            sub:
              gross > 0 ? `${((net / gross) * 100).toFixed(1)}% margin` : "—",
            color: net >= 0 ? T.ok : T.bad,
          },
          {
            label: `Expenses · ${suffix}`,
            value: fmtPeso(expenses),
            sub: "logged",
            color: T.bad,
          },
          {
            ...
            sub: voidedCount > 0 ? fmtPeso(voidedAmount) : "none",
            color: voidedCount > 0 ? T.bad : T.textMute,
          },
          {
            label: "Bar Turn Time",
      ...
      >
              ...
              >
                {topItems.length === 0 ? (
                  ...
                  topItems.map((item, i) => {
                    ...
                    const margin =
                      item.rev > 0
                        ? ((item.rev - item.cost) / item.rev) * 100
                        : null;
                    return (
                      <div
                        key={item.name}
                      ...
                      >
                        ...
                        {item.cost > 0 && (
                          <div
                            style={{ display: "flex", gap: 8, paddingLeft: 24 }}
                          >
                            <span
                              style={{
                                fontFamily: T.mono,
        ...
        >
          ...
          >
            <SectionHd
              title={`Sales by Category · ${suffix}`}
              badge={fmtPeso(gross)}
            />
            {isMobile ? (
              <div
                ...
                <div style={{ minWidth: 360 }}>
                  <HBarChart
                    ...
                    data={catBreakdown.map((c) => ({
                      ...
                      sub:
                        c.gross > 0
                          ? `${((c.net / c.gross) * 100).toFixed(0)}% net`
                          : undefined,
                    }))}
                  />
                </div>
