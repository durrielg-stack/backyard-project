...
export function selectSeniorPwdUnits(
  ...
  for (const l of lines) {
    ...
    for (let i = 0; i < l.qty; i++)
      units.push({ lineId: l.lineId, unitPrice: l.unitPrice });
  }
...
export function computeSeniorPwdDiscount(
  ...
  return selectSeniorPwdUnits(lines, count).reduce(
    (sum, u) => sum + (u.unitPrice - seniorPwdUnitPrice(u.unitPrice)),
    0,
  );
}

// Owner/Employee: whole order, every line, charged at cost (₱0 if never costed).
export function computeOwnerEmployeeDiscount(
  lines: DiscountableLine[],
): number {
  return lines.reduce(
    (sum, l) => sum + (l.unitPrice - (l.unitCost ?? 0)) * l.qty,
    0,
  );
}
...
export function computeDiscount(
  ...
  if (type === "senior_pwd")
    return computeSeniorPwdDiscount(lines, seniorCount);
  return 0;
}
