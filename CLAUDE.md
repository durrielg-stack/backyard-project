...
5. `memory-bank/engineering-judgment.md` — decision frameworks and the reasoning behind the rules; **read before any change touching money, historical data, or inventory**
6. `memory-bank/skill-financial-data-integrity.md` — loadable skill: snapshot-vs-derive, silent-failure defenses, verification discipline for the live production DB; **load before money/inventory/report work**
7. `memory-bank/skill-owner-intent-translation.md` — loadable skill: turning the owner's requests/feedback into correct behavior; vocabulary collisions, ask-vs-decide, correction generalization; **load before designing features or interpreting feedback**
8. `memory-bank/architecture.md` — component hierarchy, data flow, styling system
9. `memory-bank/database-schema.md` — all tables, columns, and types with notes
10. `memory-bank/coding-patterns.md` — reusable patterns (theme, scroll, optimistic updates, visualViewport)
11. `memory-bank/feature-map.md` — every feature mapped to its files
12. `memory-bank/active-work.md` — pending features and pre-launch scope
13. `memory-bank/changelog.md` — major milestones
