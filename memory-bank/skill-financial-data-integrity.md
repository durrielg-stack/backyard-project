...
- **Inventory automation:** sales deduct via `deduct_inventory` RPC (bucket compositions resolved through `inventory_compositions`); voids restore via `restore_inventory`; beer expenses restock via a DB trigger on `daily_expenses` (`menu_item_id` + `inventory_qty`). Beer rows' manual adjust buttons are deliberately disabled — do not re-enable them; extend the reconciliation path instead if correction is needed.
