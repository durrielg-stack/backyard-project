...
### Tables stranded as "occupied" after the last item was removed (fixed 2026-07-21)
**What:** Voiding the last item on a table (`voidItem`), paying off the last unpaid line (`payPartial`), or moving the last remaining lines to another table (`moveItems`) all auto-close the order and free the table. But each one nulled local `orderId`/`lines` state unconditionally, even when the `orders.status = 'closed'` write failed. The DB row stayed `open`, the client believed it was closed, and the table showed occupied forever with no items in the cart to void it out of.
**Why it breaks:** Local optimistic state and DB state can diverge silently — a failed write with no thrown exception (RLS reject, network blip) leaves a table permanently stuck with no user-facing way to un-stick it, since the cart that would let staff void something out is already empty.
**Fix:** Two layers. (1) `useOrder.ts` — a shared `closeOrderAndFreeTable(orderId)` helper that writes `orders.status = 'closed'` first, returns `false` and surfaces `setError` if that write fails, and only then do `voidItem`/`payPartial`/`moveItems` null out `orderId`/`lines`. (2) `useOpenOrders.ts` — a defensive net: an order that reaches `status = 'open'` with zero non-voided `order_items` rows is filtered out of the `orders` list feeding `useAutoStatus`, since a genuinely active order always has at least one item (orders are only ever created via the first `addItem`). This only applies when the items fetch itself succeeded, so a transient query error can't wrongly free every occupied table.
**Where:** `src/hooks/useOrder.ts` (`closeOrderAndFreeTable`, `voidItem`, `payPartial`, `moveItems`), `src/hooks/useOpenOrders.ts` (`ordersWithItems` filter). `useAutoStatus.ts` needed no change — it already derives `occupied` from `useOpenOrders`'s `orders` list, so the filter there is enough.
...
### Never write versioned data to prod before the code that reads it is deployed (2026-09-21)
**What:** Raising Salary from ₱1,950 to ₱2,250 for September onward. The effective-dating change (`opex_items.effective_from`/`effective_to`) was written to the production DB — schema migration plus the split into two Salary rows — while the code that filters by those columns was still uncommitted on a working tree. The deployed build was still the date-unaware `for (const item of items)` loop.
**Why it breaks:** The old code summed **both** Salary versions: ₱1,950 + ₱2,250 = ₱4,200/day. September OPEX read ₱169,975 instead of ₱121,225, and every month was inflated. The owner caught it in the live app. Adding a row to a table an old build reads without a filter is a silent double-count, not a no-op — unlike adding a *column*, which old code ignores.
**Why the verification missed it:** the numbers were checked with SQL that mirrored the *new* rule and matched the golden figures exactly. Correct rule, wrong system — nothing verified what the *currently deployed* build would compute from the new rows. A golden-number check that models the code you just wrote instead of the code that is running proves nothing about production.
**Fix:** On any derived-money path, data and the code that interprets it ship together — deploy the code first, or in the same window, never data first. Before writing rows that only new code understands, ask what `HEAD` (not the working tree) does with them. Verify against the deployed build: re-query after the deploy is `READY`, and where possible model the old code's query too as an explicit "what breaks if this doesn't ship" check.
**Interim when it does happen:** deactivating the superseded version (`is_active = false`) stops the double-count immediately under old code, at the cost of temporarily restating history; reactivate it once the date-aware build is live. Also note OPEX has no stored snapshot — allocation is derived on every load, so a bad row is wrong *everywhere* the instant it lands, with no cached value shielding closed months.
